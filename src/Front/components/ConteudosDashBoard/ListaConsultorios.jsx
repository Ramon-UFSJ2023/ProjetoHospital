...
export default function ListaConsultorios() {
  ...
  const fetchConsultorios = async (termo) => {
    if (!token) return;
    try {
      const response = await fetch(
        `http://localhost:3001/api/admin/buscar-consultorios?busca=${termo}`,
  ...
  useEffect(() => {
    if (token) fetchConsultorios("");
  }, [token]);

  const handleSearch = (e) => {
    e.preventDefault();
    fetchConsultorios(searchTerm);
  };

  const handleExcluir = async () => {
    if (!itemSelecionado) return;
    if (!window.confirm(`Excluir Consultório ${itemSelecionado.Numero} do Bloco ${itemSelecionado.Bloco}?`)) return;

    try {
      const response = await fetch('http://localhost:3001/api/admin/consultorios/deletar', {
        method: 'DELETE',
        headers: {
    ...
    <div className="container-conteudo-admin">
        ...
        <div className="modal-overlay" onClick={() => setItemSelecionado(null)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2>Detalhes do Consultório</h2>
            </div>
            <div className="modal-body">
              <p><strong>Número:</strong> {itemSelecionado.Numero}</p>
              <p><strong>Especialidade:</strong> {itemSelecionado.Especialidade || 'Geral'}</p>
      ...
      <form className="busca-container" onSubmit={handleSearch}>
        <input
          ...
          placeholder="Buscar por Número, Especialidade ou Bloco..."
          value={searchTerm}
        ...
        <table className="tabela-resultados">
          ...
          <tbody>
            {resultados.length > 0 ? (
              ...
              <tr><td colSpan="5" style={{textAlign:'center'}}>Nenhum consultório encontrado.</td></tr>
            )}
          </tbody>
        </table>
      </div>
