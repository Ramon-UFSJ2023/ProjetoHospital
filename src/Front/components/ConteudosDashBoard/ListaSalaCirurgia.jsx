...
export default function ListaSalaCirurgia() {
  ...
  const fetchSalas = async (termo) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3001/api/admin/buscar-salas-cirurgia?busca=${termo}`,
  ...
  const handleExcluir = async () => {
    ...
    if (!window.confirm(`Excluir Sala ${itemSelecionado.N_Sala} do Bloco ${itemSelecionado.Bloco}?`)) return;

    try {
      const response = await fetch('http://localhost:3001/api/admin/salas-cirurgia/deletar', {
        method: 'DELETE',
        headers: {
        ...
        <div className="modal-overlay" onClick={() => setItemSelecionado(null)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2>Detalhes da Sala Cirúrgica</h2>
            </div>
            <div className="modal-body">
              <p><strong>Número:</strong> {itemSelecionado.N_Sala}</p>
              <p><strong>Capacidade:</strong> {itemSelecionado.Capacidade} pessoas</p>
